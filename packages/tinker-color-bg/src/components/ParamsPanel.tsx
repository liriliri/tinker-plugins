...
const ParamsPanel = observer(() => {
  ...
  return (
    <aside className="cb-panel cb-panel-right">
      ...
      <footer className={`shrink-0 px-3.5 py-3 ${tw.surface.footer}`}>
        <div className="flex gap-2 mb-2.5">
          <label className="flex-1 flex flex-col gap-1">
            <span className={`text-[11px] ${tw.text.secondary}`}>
              {t('width')}
            </span>
            <input
              type="number"
