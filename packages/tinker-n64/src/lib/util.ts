import escapeJsStr from 'licia/escapeJsStr'
import type { PlayerKeymap } from './keymap'

export function buildIframeHtml(
  gameUrl: string,
  gameName: string,
  baseUrl: string,
  loadingText: string,
  keymap: PlayerKeymap,
) {
  const escapedGameUrl = escapeJsStr(gameUrl)
  const escapedGameName = escapeJsStr(gameName)
  const escapedBaseUrl = escapeJsStr(baseUrl)
  const escapedLoadingText = escapeJsStr(loadingText)
  const escapedKeymap = escapeJsStr(JSON.stringify(keymap))

  return `<!DOCTYPE html>
<html>
<head>
<style>
html,body{margin:0;padding:0;overflow:hidden;background:#000;user-select:none;-webkit-user-select:none;-webkit-tap-highlight-color:transparent;}
.container{position:fixed;inset:0;display:flex;justify-content:center;align-items:center;background:#000;user-select:none;}
#loading{position:absolute;inset:0;background:#000;color:#fff;display:flex;align-items:center;justify-content:center;font:18px Menlo,monospace;z-index:10;}
#canvas{aspect-ratio:4/3;height:100%!important;max-width:100%;max-height:100%;outline:none;user-select:none;-webkit-user-select:none;}
</style>
</head>
<body>
<script>
navigator.getGamepads = () => [];
window.addEventListener('gamepadconnected', e => e.stopImmediatePropagation(), true);
window.addEventListener('gamepaddisconnected', e => e.stopImmediatePropagation(), true);
</script>
<div class="container">
  <div id="loading">${escapedLoadingText}</div>
  <canvas id="canvas" tabindex="0"></canvas>
</div>
<script>
window.gameUrl='${escapedGameUrl}';
window.gameName='${escapedGameName}';
window.baseUrl='${escapedBaseUrl}';
window.keymapJson='${escapedKeymap}';
</script>
<script type="module" src="${baseUrl}bootstrap.js"></script>
</body>
</html>`
}
