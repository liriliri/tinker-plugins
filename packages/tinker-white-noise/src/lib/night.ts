import night from '../assets/night.m4a'

// https://www.shadertoy.com/view/4s33zf
const shader = `// "Over the Moon" by Martijn Steinrucken aka BigWings - 2015
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  for (float i = 0.; i < 10.; i++) {
    ...
    layer.rgb *= mix(vec3(.05, .15, .05), vec3(.1, .3, .1) + gradient(uv).x * .5, 1. - i / 10.);
  ...
  foreground.rgb *= vec3(.05, .15, .05) * .5;
...
export default { audio: night, renderPass }
